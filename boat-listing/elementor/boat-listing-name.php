...
class boat_listing_name extends \ElementorPro\Modules\Forms\Fields\Field_Base {
    ...
    public function __construct() {
        ...
        //$this->boat_names = $helper->fetch_all_boats(); // store for later
    }
    ...
    public function get_name() {
        return esc_html__( 'Boat Name', BL_TEXT_DOMAIN );
    }

    public function render( $item, $item_index, $form ): void {
        
        // Debug: Let's see what's in the $item array
        // error_log('Boat Name Field item data: ' . print_r($item, true));
        ...
        if ( ! empty( $item['required'] ) ) {
            $form->add_render_attribute( 'select_boat_name' . $item_index, 'required', 'required' );
            $form->add_render_attribute( 'select_boat_name' . $item_index, 'aria-required', 'true' );
        }

        echo '<select ' . $form->get_render_attribute_string( 'select_boat_name' . $item_index ) . '>';

        // Default option - Fixed the inconsistent text
        $placeholder = ! empty( $item['placeholder'] ) ? $item['placeholder'] : esc_html__( 'Select boat name', BL_TEXT_DOMAIN );
        echo '<option value="">' . esc_html( $placeholder ) . '</option>';

        if ( ! empty( $this->boat_names ) && is_array( $this->boat_names ) ) {
            foreach ( $this->boat_names as $boat_name ) {
                $data = $boat_name['data'];
                $value = $data['name'];
                $selected = '';
        ...
        jQuery( document ).ready( () => {
            elementor.hooks.addFilter(
                ...
                function ( inputField, item, i ) {
                    ...
                    const placeholder = item.placeholder || '<?php echo esc_js( __( 'Select boat name', BL_TEXT_DOMAIN ) ); ?>';

                    // Default option
                    let options = `<option value="">${placeholder}</option>`;

                    <?php if ( ! empty( $this->boat_names ) && is_array( $this->boat_names ) ) : ?>
                        <?php foreach ( $this->boat_names as $boat_name ) :
                            $data = $boat_name['data'];
